...
public class ArrayListExample {
     ...
     * @param flat_shapes
     */
    public static double total_area(ArrayList<? extends TwoDShape> flat_shapes)
    {
        double area =0;
        for (TwoDShape g: flat_shapes)
        {
            area += g.area();
        }
        return area;
    ...
    public static double total_perimeter(ArrayList<Rectangle> shape)
    {
        double perimeter =0;
        for (Rectangle g: shape)
        {
     ...
     */
    public static void describe_all(ArrayList<? extends GeometricShape> shape)
    {
        int i=0;
        for (GeometricShape g: shape)
        {
            g.describe();
            i++;
        }
     ...
     *      {@code new Sphere(0.0);          }
     */
    public static void add_empties(ArrayList<GeometricShape> shape)
    {
        shape.add(new Circle(0.0));
        shape.add(new Cone(0.0, 0.0));
        shape.add(new Rectangle(0.0, 0.0));
        shape.add(new Sphere(0.0));
    }
    ...
    public static <T extends GeometricShape> ArrayList<T> supersize_list(ArrayList<? extends GeometricShape> shape)
    {
        ArrayList<T> list = new ArrayList<>();
        for (GeometricShape g: shape)
        {
            list.add(g.supersize());
        }
        return list;
    }
